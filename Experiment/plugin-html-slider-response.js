var jsPsychHtmlSliderResponse = (function (jspsych) {
  ...
  class HtmlSliderResponsePlugin {
      ...
      trial(display_element, trial) {
          ...
          if (trial.prompt2 == null) { // playing actor
          html += trial.prompt1
          html += '<div id="jspsych-html-slider-response-wrapper" style="margin: 7px 0px;">';
          html +=
          ...
          html += "</div>";
          html += "</div>";
          } else { // playing observer
          // first prompt + slider          
          html += trial.prompt1;

          html += '<div class="column" style="float: right;">';
          html += '<div id="jspsych-html-slider-response-wrapper" style="margin: 7px 0px;">';
          html +=
              '<div class="jspsych-html-slider-response-container" style="position:relative; margin: 0 auto 3em auto; ';
          if (trial.slider_width !== null) {
          ...
          // original response1
          if (trial.original_response1 != null) {
            var percent_dist_from_center = ((trial.original_response1 - 50) / 50) * 100;
            var offset = (percent_dist_from_center * half_thumb_width) / 100;
            html +=
                    '<div style="border: 1px solid transparent; display: inline-block; position: absolute; ' +
                        "left:calc(" +
                        trial.original_response1 +
                        "% - (" +
                        trial.original_response1 +
                        "% / 2) - " +
                        offset +
                        ...
                        trial.original_response1 +
                        '%;">';
            html += '<span style="text-align: center; font-size: 80%; color: gray">' + '<b>| <br> your response</b>' + "</span>";
            html += "</div>";
          }
          html += '</div>';
          html += '</div>';
          ...
          // original response2
          if (trial.original_response2 != null) {
            var percent_dist_from_center = ((trial.original_response2 - 50) / 50) * 100;
            var offset = (percent_dist_from_center * half_thumb_width) / 100;
            html +=
                    '<div style="border: 1px solid transparent; display: inline-block; position: absolute; ' +
                        "left:calc(" +
                        trial.original_response2 +
                        "% - (" +
                        trial.original_response2 +
                        "% / 2) - " +
                        offset +
          ...
          html +=
              '<input type="range" class="jspsych-slider" value="' +
                  ...
                  '" step="' +
                  trial.step +
                  '" id="jspsych-html-slider-response-response3"></input>';
          html += '<div>';
          for (var j = 0; j < trial.labels.length; j++) {
              var label_width_perc = 100 / (trial.labels.length - 1);
              var percent_of_range = j * (100 / (trial.labels.length - 1));
          ...
          // original response3
          if (trial.original_response3 != null) {
            var percent_dist_from_center = ((trial.original_response3 - 50) / 50) * 100;
            var offset = (percent_dist_from_center * half_thumb_width) / 100;
            html +=
                    '<div style="border: 1px solid transparent; display: inline-block; position: absolute; ' +
                        "left:calc(" +
                        trial.original_response3 +
                        "% - (" +
                        trial.original_response3 +
                        "% / 2) - " +
                        offset +
          ...
          html +=
              '<input type="range" class="jspsych-slider" value="' +
                  ...
                  '" step="' +
                  trial.step +
                  '" id="jspsych-html-slider-response-response4"></input>';
          html += '<div>';
          for (var j = 0; j < trial.labels.length; j++) {
              var label_width_perc = 100 / (trial.labels.length - 1);
              var percent_of_range = j * (100 / (trial.labels.length - 1));
          ...
          // original response4
          if (trial.original_response4 != null) {
            var percent_dist_from_center = ((trial.original_response4 - 50) / 50) * 100;
            var offset = (percent_dist_from_center * half_thumb_width) / 100;
            html +=
                    '<div style="border: 1px solid transparent; display: inline-block; position: absolute; ' +
                        "left:calc(" +
                        trial.original_response4 +
                        "% - (" +
                        trial.original_response4 +
                        "% / 2) - " +
                        offset +
          ...
          var record_click1 = false;
          var record_click2 = false;
          var record_click3 = false;
          var record_click4 = false;
          ...
          if (trial.require_movement) {
              ...
              display_element
                  ...
                  .addEventListener("mousedown", function() {
                    record_click1 = true;
                    if (record_click1 && record_click2 && record_click3 && record_click4) {
                      enable_button()
                    }
                  });
              display_element
              ...
              display_element
                  .querySelector("#jspsych-html-slider-response-response3") 
                  .addEventListener("mousedown", function() {
                    record_click3 = true;
                    if (record_click1 && record_click2 && record_click3 && record_click4) {
                      enable_button()
              ...
              display_element
                  .querySelector("#jspsych-html-slider-response-response4") 
                  .addEventListener("mousedown", function() {
                    record_click4 = true;
                    if (record_click1 && record_click2 && record_click3 && record_click4) {
                      enable_button()
          ...
          const end_trial = () => {
              ...
              var trialdata = {
                  ...
                  slider_start3: trial.slider_start3,
                  slider_start4: trial.slider_start4,
      ...
      create_simulation_data(trial, simulation_options) {
          const default_data = {
              ...
              slider_start3: trial.slider_start3,
              slider_start4: trial.slider_start4,
              response1: this.jsPsych.randomization.randomInt(trial.min, trial.max),
              response2: this.jsPsych.randomization.randomInt(trial.min, trial.max),
              response3: this.jsPsych.randomization.randomInt(trial.min, trial.max),
              response4: this.jsPsych.randomization.randomInt(trial.min, trial.max),
