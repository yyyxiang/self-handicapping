...
parse_posterior <- function(s) {
  as.numeric(strsplit(s, ';')[[1]])
}

kl_divergence_plot <- function(data, exp) {
  
  p <- data %>% 
    filter(exp_index == paste0('exp',exp)) %>%
    ggplot(aes(outcome, divergence, color = outcome)) +
    geom_point(aes(shape = 'Model'), size = 2, stroke = 2, fill = 'white') +
    scale_color_brewer(palette = 'Set1') +
    scale_shape_manual(values = c('Model' = 24), name = NULL) +
    guides(color = 'none') +
    scale_x_discrete(labels = str_to_title(posterior_df$outcome)) +
    coord_cartesian(ylim = c(0, 0.20)) +
    labs(x = 'Outcome', y = 'KL-Divergence') +
    theme_classic() +
    theme(
      axis.title = element_text(size = 16),
      axis.text = element_text(size = 14), 
